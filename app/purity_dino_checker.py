...
Case 3 flow (ported from the latest sample_dino.py, tuned thresholds):
    ...
    * Signage-strip and degenerate full-frame boxes are dropped from all
      passes first (printed branding strips misread as "lunch box"/"box",
      and near-whole-image boxes from a weakly-matched phrase).
    * NEW: cross-theme dedup across the specific passes THEMSELVES, before
      generic is even considered. Different specific themes (e.g. "food"
      and "produce") can each independently land a box on the SAME
      physical object at very high IoU (observed 0.93-0.99 in production
      logs, e.g. 'drinking glass' / 'detergent packet' / 'plastic bag' all
      on one real item) — there was previously no dedup between specific
      themes, only between generic and specific. Boxes are clustered by
      IoU alone (no containment — two genuinely distinct, closely-packed
      objects can have high containment without being duplicates) at a
      strict default of 0.90 (cross_theme_dedupe_iou), keeping only the
      single highest-DINO-confidence box per cluster — never a fixed
      theme/dict-order winner. See _cross_theme_dedupe_specific().
    * The passes are merged with generic boxes dropped if they overlap a
      specific-theme box (same physical object, less useful label). This
      generic-vs-specific merge step is unchanged by the addition above.
    * Any merged box overlapping a beverage bounding box (IoU or
      containment, tuned thresholds) is dropped — it's part of a
      beverage, not an impurity.
    * Any remaining box that has a KNOWN-BRAND cap sitting inside it is
...
# Cross-theme specific-vs-specific dedup — NEW, separate from the
# generic-vs-specific dedup above (which is unchanged). Different
# specific-theme prompts can each independently box the same physical
# object at very high IoU. Deliberately strict at first: two genuinely
# distinct objects sitting close together on a shelf can also produce a
# high IoU without being the same object, so a high bar minimizes the
# risk of collapsing two real neighboring items into one. IoU-only (no
# containment) — see _cross_theme_dedupe_specific(). Tune down later via
# dino_cfg["cross_theme_dedupe_iou"] if duplicates persist above this bar.
DEFAULT_CROSS_THEME_DEDUPE_IOU = 0.90
...
def _cross_theme_dedupe_specific(boxes_with_phrase_score, iou_thresh, filename=None):
    """
    Cross-theme dedup for specific-theme detections ONLY, run BEFORE the
    (unchanged) generic-vs-specific merge in check_purity_case3(). Two
    different specific-theme prompts (e.g. "food" and "produce") can each
    independently land a box on the SAME physical object at very high
    IoU — observed in production logs at iou=0.93-0.99 between phrases
    like 'drinking glass', 'detergent packet', 'plastic bag' all landing
    on one real object — because there is no dedup between specific
    themes today, only between generic and specific (that logic is
    untouched by this function).

    Rules (per spec):
      * IoU only. Containment is deliberately NOT used here — two
        genuinely distinct objects placed close together on a shelf can
        have high containment (one box partly inside another) without
        being the same object, so containment would be too aggressive
        for this specific check.
      * IoU >= iou_thresh -> treated as duplicates, keep exactly one.
      * IoU <  iou_thresh -> both kept.
      * Among duplicates, keep the box with the highest DINO
        confidence/logit. Theme/dict order is never used to decide.
      * Boxes are clustered transitively via union-find, so a chain
        A-B-C (only adjacent pairs individually clearing iou_thresh)
        still collapses into one cluster instead of leaving a dangling
        duplicate.

    boxes_with_phrase_score: list of (box, phrase, score) tuples, box as
    [x1, y1, x2, y2] absolute pixel coords, score the raw DINO
    confidence/logit for that box. Returns the same shape, deduped.
    """
    n = len(boxes_with_phrase_score)
    if n < 2:
        return boxes_with_phrase_score
    ...
    for i in range(n):
        for j in range(i + 1, n):
            box_i = boxes_with_phrase_score[i][0]
            box_j = boxes_with_phrase_score[j][0]
    ...
    for root, idxs in clusters.items():
        if len(idxs) == 1:
            kept.append(boxes_with_phrase_score[idxs[0]])
            continue

        # Highest DINO confidence/logit wins — never theme/dict order.
        best_idx = max(idxs, key=lambda i: boxes_with_phrase_score[i][2])
        kept.append(boxes_with_phrase_score[best_idx])

        best_box, best_phrase, best_score = boxes_with_phrase_score[best_idx]
        for i in idxs:
            if i == best_idx:
                continue
            d_box, d_phrase, d_score = boxes_with_phrase_score[i]
            logger.info(
                f"[DINO-DEBUG] {filename or ''}: cross-theme dedupe "
                f"(iou>={iou_thresh}) dropped {d_phrase!r}{tuple(d_box)} "
                f"score={d_score:.4f}  kept {best_phrase!r}{tuple(best_box)} "
...
# ── DIAGNOSTIC ONLY: box/coordinate/overlap dumper ─────────────────────────
# Pure logging helper — does not alter any detection, filtering, merging,
# or verdict logic anywhere in this module. Added solely so overlapping/
# duplicate bounding boxes on the same physical object can be inspected
# (coordinates, area, phrase, and pairwise IoU/containment) before any
# dedup logic is designed. Safe to leave in or strip out later; it never
# changes control flow or return values.
def _debug_dump_boxes(label, boxes_with_phrase, W=None, H=None):
    """
    boxes_with_phrase: list of (box, phrase) tuples, where box is
    [x1, y1, x2, y2] (or a 4-tuple) in absolute pixel coordinates.

    Logs, for `label`:
      - every box's coordinates, area fraction of the frame (if W/H
        given), and its phrase/name
      - every pair of boxes that have ANY IoU or containment overlap
        (> 0), with the exact IoU and containment values, so you can
        see which pairs are candidates for "same object, duplicate box"
        before choosing a dedup threshold.
    """
    logger.info(f"[DINO-DEBUG] --- {label}: {len(boxes_with_phrase)} box(es) ---")
    for idx, (box, phrase) in enumerate(boxes_with_phrase):
        x1, y1, x2, y2 = box
        area = max(0, x2 - x1) * max(0, y2 - y1)
        if W and H and (W * H) > 0:
            frac_str = f"{area / (W * H):.4f}"
        else:
            frac_str = "n/a"
        logger.info(
            f"[DINO-DEBUG]   [{idx}] box=({x1}, {y1}, {x2}, {y2})  "
            f"area_frac={frac_str}  phrase={phrase!r}"
        )

    n = len(boxes_with_phrase)
    if n < 2:
        return

    logger.info(f"[DINO-DEBUG]   -- pairwise overlaps within '{label}' --")
    found_any = False
    for i in range(n):
        for j in range(i + 1, n):
            box_i, phrase_i = boxes_with_phrase[i]
            box_j, phrase_j = boxes_with_phrase[j]
            iou_val = iou(box_i, box_j)
            cont_val = containment_ratio(box_i, box_j)
            if iou_val > 0 or cont_val > 0:
                found_any = True
                logger.info(
                    f"[DINO-DEBUG]     [{i}]<->[{j}]  "
                    f"{phrase_i!r}{tuple(box_i)}  overlaps  "
                    f"{phrase_j!r}{tuple(box_j)}  "
                    f"iou={iou_val:.4f}  containment={cont_val:.4f}"
                )
    if not found_any:
        logger.info(f"[DINO-DEBUG]     (no overlapping pairs in '{label}')")
...
def _run_grounding(model, image_tensor, prompt, box_thresh, text_thresh, W, H):
    """
    Returns list of (box, phrase, score) — score is the raw DINO
    confidence/logit for that box, now also returned (previously
    discarded) so the new cross-theme dedup below can pick the
    highest-confidence box among duplicates. This is a shared helper used
    for both the specific and generic passes; the generic-vs-specific
    merge logic in check_purity_case3() strips the score back off
    immediately after calling this for the generic pass, so that merge
    step's own behavior is unchanged.
    """
    from groundingdino.util.inference import predict
    ...
    abs_boxes = []
    for b, score, phrase in zip(boxes, logits, phrases):
        cx, cy, w, h = b.cpu().numpy()
        x1 = int((cx - w / 2) * W)
        y1 = int((cy - h / 2) * H)
        x2 = int((cx + w / 2) * W)
        y2 = int((cy + h / 2) * H)
        abs_boxes.append(([x1, y1, x2, y2], phrase, float(score)))
...
def check_purity_case3(local_path: str, filename: str, beverage_boxes: list,
    ...
    try:
        ...
        # [DINO-DEBUG] caller-supplied reference boxes (beverage / known
        # caps) for this image — logged once up front so every later
        # overlap dump can be visually cross-checked against them.
        _debug_dump_boxes(
            f"{filename}: INPUT beverage_boxes",
            [(b, "Beverage") for b in beverage_boxes], W, H,
        )
        _debug_dump_boxes(
            f"{filename}: INPUT known_cap_boxes",
            [(b, "Known Cap") for b in known_cap_boxes], W, H,
        )
        ...
        max_box_area_frac = dino_cfg.get("max_box_area_frac", DEFAULT_MAX_BOX_AREA_FRAC)
        min_region_std = dino_cfg.get("min_region_std", DEFAULT_MIN_REGION_STD)
        cross_theme_dedupe_iou = dino_cfg.get("cross_theme_dedupe_iou", DEFAULT_CROSS_THEME_DEDUPE_IOU)
        ...
        specific_boxes = []  # list of (box, phrase, score) until cross-theme dedupe below
        for theme_name, theme_prompt in active_specific_prompts.items():
            box_t, text_t = theme_overrides_cfg.get(
                theme_name,
                DEFAULT_THEME_THRESHOLD_OVERRIDES.get(
            ...
            theme_boxes = _run_grounding(
                grounding_model, image_tensor, theme_prompt,
                box_t, text_t, W, H,
            )
            # [DINO-DEBUG] raw boxes for this theme alone, before any
            # merging with other themes — lets you see which theme(s)
            # are producing a box on the same object as another theme.
            _debug_dump_boxes(
                f"{filename}: theme='{theme_name}' RAW (box_t={box_t}, text_t={text_t})",
                [(b, p) for b, p, s in theme_boxes], W, H,
            )
            specific_boxes.extend(theme_boxes)

        # Generic pass: _run_grounding now returns (box, phrase, score),
        # but the generic-vs-specific merge below is UNCHANGED and still
        # expects plain (box, phrase) — strip the score off immediately
        # so that merge logic's behavior/inputs are identical to before.
        generic_boxes_raw = _run_grounding(
            grounding_model, image_tensor, GROUNDING_PROMPT_GENERIC,
            generic_thresh, generic_text_thresh, W, H,
        )
        generic_boxes = [(b, p) for b, p, s in generic_boxes_raw]
        _debug_dump_boxes(f"{filename}: GENERIC RAW", generic_boxes, W, H)

        # Drop signage-strip and degenerate full-frame false positives
        # from BOTH prompts before anything else touches them.
        # Unchanged — still operates on plain (box, phrase) tuples, still
        # used as-is for generic_boxes below.
        def _strip_degenerate(boxes_with_phrase):
            kept = []
            for gbox, phrase in boxes_with_phrase:
                if looks_like_signage_strip(gbox, W, H):
        ...
        # Same strip logic as _strip_degenerate, but keeps the score
        # column so it survives into the new cross-theme dedupe step.
        def _strip_degenerate_scored(boxes_with_phrase_score):
            kept = []
            for gbox, phrase, score in boxes_with_phrase_score:
                if looks_like_signage_strip(gbox, W, H):
                    continue
                if is_degenerate_fullframe_box(gbox, W, H, max_box_area_frac):
                    continue
                kept.append((gbox, phrase, score))
            return kept

        specific_boxes = _strip_degenerate_scored(specific_boxes)
        generic_boxes = _strip_degenerate(generic_boxes)

        # [DINO-DEBUG] ALL specific-theme boxes combined (across every
        # theme) after the signage/full-frame strip, BEFORE cross-theme
        # dedupe. Any pairwise overlap printed here, from two DIFFERENT
        # themes, is a same-object double-box coming from the theme split.
        _debug_dump_boxes(
            f"{filename}: SPECIFIC combined, post-strip (pre cross-theme-dedupe)",
            [(b, p) for b, p, s in specific_boxes], W, H,
        )

        # NEW: cross-theme dedup — IoU only, strict threshold, keeps the
        # highest-confidence box per duplicate cluster. Scoped ONLY to
        # the specific-theme boxes; does not touch generic_boxes or the
        # generic-vs-specific merge below. See _cross_theme_dedupe_specific().
        specific_boxes = _cross_theme_dedupe_specific(
            specific_boxes, cross_theme_dedupe_iou, filename=filename,
        )

        # Score no longer needed past this point — convert back to plain
        # (box, phrase) so every line below (merge, beverage-exclusion,
        # cap-rescue, low-contrast filter, hard override, drawing) is
        # exactly the same code operating on exactly the same shape of
        # data as before this change.
        specific_boxes = [(b, p) for b, p, s in specific_boxes]

        _debug_dump_boxes(
            f"{filename}: SPECIFIC combined, post cross-theme-dedupe (pre-merge)",
            specific_boxes, W, H,
        )
        _debug_dump_boxes(
            f"{filename}: GENERIC post-strip",
            generic_boxes, W, H,
        )
        ...
        # [DINO-DEBUG] full merged set (specific + surviving generic)
        # handed to the beverage-exclusion step below.
        _debug_dump_boxes(f"{filename}: MERGED (specific + surviving generic)", merged_boxes, W, H)
        ...
        # [DINO-DEBUG] what's left after beverage-box exclusion — this is
        # the set that feeds the cap-rescue / low-contrast / impure loop
        # below. Overlaps printed here are candidates that will become
        # duplicate impure boxes on the same object unless deduped.
        _debug_dump_boxes(f"{filename}: UNKNOWN (post beverage-exclusion)", unknown, W, H)
        ...
        # [DINO-DEBUG] final impure box set (DINO-caught + any hard-
        # override flagged hits) — this is what actually gets drawn red
        # on the annotated debug image and reported as impure. Any
        # overlapping pair here is a literal duplicate box on the same
        # object in the final result.
        _debug_dump_boxes(f"{filename}: FINAL impure_boxes (incl. hard override)", impure_boxes, W, H)
        _debug_dump_boxes(f"{filename}: rescued_boxes (dropped via known-cap rescue)", rescued_boxes, W, H)
        logger.info(f"[DINO-DEBUG] {filename}: verdict={verdict}  nonbeverage_detections={nonbeverage_detections}")
